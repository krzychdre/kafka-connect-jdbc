...
public class KafkaTimestampAppender implements SinkRecordTransformer {

  public static final String KAFKA_TIMESTAMP = "_kafka_timestamp";
  ...
  public SinkRecord transform(SinkRecord record) {
    ...
    Schema recordSchema = baseAppender.rewriteAllFields(record)
        .field(KAFKA_TIMESTAMP, Schema.OPTIONAL_INT64_SCHEMA).build();

    Struct recordValues = baseAppender.rewriteAllFieldsValues(record, recordSchema);
    baseAppender.setFieldValue(recordValues, KAFKA_TIMESTAMP, record.timestamp());
